...
exports.hireUser = async(req, res) => {
        ...
        try{
            ...
            const currentEstablisment = await adminModel.findOne({ _id : currentSupervisor.establisment},{ hirings : 1, users : 1, hireds : 1})
            .populate('hirings')
...
exports.addReportingLocation = async (req, res) => {
  try {
    ...
    if (!latitude || !longitude || !areaName || !reportingRadius) {
      return res.status(400).json({
        success: false,
        message: 'Please provide latitude, longitude, area name and reporting radius'
      });
    }
